...
static void usb_rx_task(void *arg) {
    ...
    // Buffer for collecting a morse character (e.g. ".-" before space)
    char morse_buf[8] = {0};
    int morse_len = 0;
        ...
        if (tud_cdc_n_connected(CDC_ITF_TX) && tud_cdc_n_available(CDC_ITF_TX)) {
            ...
            if (count > 0) {
                ...
                for (uint32_t i = 0; i < count; i++) {
                    ...
                    /* ───────────────────────────────────────────
                       CASE 1: MORSE INPUT (. or -)
                    ─────────────────────────────────────────── */
                    if (c == '.' || c == '-') {
                        xQueueSend(xMorseRxQueue, &c, 0);

                        if (morse_len < 7) {
                            morse_buf[morse_len++] = c;
                    ...
                    /* ───────────────────────────────────────────
                       CASE 2: MORSE SEPARATOR (space or '/')
                    ─────────────────────────────────────────── */
                    if (c == ' ' || c == '/') {
                        char gap = ' ';
                        ...
                        if (morse_len > 0) {
                            char decoded = get_char_from_morse(morse_buf);
                    ...
                    /* ───────────────────────────────────────────
                       CASE 3: LETTER / NUMBER INPUT
                    ─────────────────────────────────────────── */
                    if (isalnum((unsigned char)c)) {
                        const char* mc = get_morse_from_char(c);
                        ...
                        // Play the morse audio
                        if (mc) {
                            for (int k = 0; mc[k]; k++)
                                xQueueSend(xMorseRxQueue, &mc[k], 0);
                    ...
                    if (c == '\n') {
                        ...
                        char end = '\0';
                        xQueueSend(xMorseRxQueue, &end, portMAX_DELAY);
